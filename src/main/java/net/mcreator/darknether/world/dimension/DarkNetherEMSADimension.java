...
package net.mcreator.darknether.world.dimension;
...
import net.mcreator.darknether.item.DarkNetherEMSAItem;
import net.mcreator.darknether.block.DarkNetherCompressedObsidianBlock;
...
public class DarkNetherEMSADimension extends DarkNetherModElements.ModElement {
	@ObjectHolder("dark_nether:dark_nether_emsa")
	public static final ModDimension dimension = null;
	@ObjectHolder("dark_nether:dark_nether_emsa_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public DarkNetherEMSADimension(DarkNetherModElements instance) {
		super(instance, 1);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("dark_nether_emsa"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("dark_nether:dark_nether_emsa")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("dark_nether:dark_nether_emsa"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("dark_nether:dark_nether_emsa"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new DarkNetherEMSAItem().setRegistryName("dark_nether_emsa"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("dark_nether_emsa_portal");
		}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == DarkNetherCompressedObsidianBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == DarkNetherCompressedObsidianBlock.block.getDefaultState().getBlock())
						? i
						: 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == DarkNetherCompressedObsidianBlock.block.getDefaultState()
									.getBlock())) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					...
					if (!(this.world.getBlockState(framePos).getBlock() == DarkNetherCompressedObsidianBlock.block.getDefaultState().getBlock())) {
						this.height = 0;
						break;
					}
				}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag
											? DarkNetherCompressedObsidianBlock.block.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos,
								DarkNetherCompressedObsidianBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
